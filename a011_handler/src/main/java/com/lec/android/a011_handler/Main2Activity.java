...
public class Main2Activity extends AppCompatActivity {
    ...
    @Override
    protected void onCreate(Bundle savedInstanceState) { //여기는 돌리는부분인거같음 , 여기가메인
        ...
        //방법3
        BackThread3 thread3 =new BackThread3(handler3); //여기서의 Point , 메인스레드의 Handler 객체를 외부 클레스에 넘겨줌
    ...
    class BackThread1 extends Thread{  //inner class
        @Override
        public void run() {
            while(true){
    ...
    Handler handler3 = new Handler(){
        ...
        public void handleMessage(@NonNull Message msg) {
            if(msg.what==0) {
                //그 메세지를 통해 받은 값을 메인 UI에 출력
                tvBackValue3.setText("BackValue3: "+msg.arg1);//메인스레드의 UI변경
            }
        }
    };
...
class BackThread3 extends Thread{  //작업스레드
    ...
    BackThread3(Handler handler){this.handler=handler;} //외부로부터받아서 생성하는것
    ...
    public void run() {
        while(true){
            ...
            Message msg =new Message();//메세지생성 ,여기에다가 3씩증가하게 담아서
            msg.what =0; //메세지 id (what)생성
            msg.arg1 = backValue;

            handler.sendMessage(msg); //메인 스레드의 핸들러에 메세지 보내기 ,여기서보냄
...
//방법4 :메인스레드의 Handler를 직접사용할수 없는 불리된 작업 스레드  (요즘은 잘않씀 ,별로안중요)
    ...
    public void run() {
        while(true){
          ...
          // obtain(Handler h, int what, int arg1, int arg2)
            ...
            handler.sendMessage(msg); //메인 스레드의 Handler 에 메세지보내기
