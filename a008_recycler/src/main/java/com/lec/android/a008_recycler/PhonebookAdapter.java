...
// 본 예제에서는 Adapter 안에 직접 데이터를 다루어보겠습니다
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        //XmL파일로부터 View 를 생성하여 ViewGroup에 붙이고
        LayoutInflater inf = LayoutInflater.from(parent.getContext());
        //그렇게 만들어진 view를 리턴한다
      View itemView =  inf.inflate(R.layout.item,parent,false);

        //위에서 만들어진 새로운view를 ViewHolder에 담아 리턴
...
//  리스트 상에 몇번째 데이터인지에 대한 정보 (position) 가 넘어온다
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Phonebook item = items.get(position); //Arraylist의 메서드
        holder.setItem(item);
...
//   Selection Widget 에서 수시로 getItemCount() 를 호출하여 뷰를 업데이트 한다
    ...
    static class ViewHolder extends RecyclerView.ViewHolder{
        ...
        public ViewHolder(@NonNull View itemView) { //tiem 레이아웃의 view 객체가 전달됨.
            ...
            btnDelItem= itemView.findViewById(R.id.btnDeltem);
            swOnOff= itemView.findViewById(R.id.swOnOff);

            //switch 누르면 전화번호, 이메일 숨기긱/보이기
            ...
            //클릭리스너 장착 , 이부분을클릭하면 상세 페이지로 전환하게 만들었다.
            itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    int position = getAdapterPosition(); //이 리스너가 장착된 item 의 리스트상의 position 값
                    //Toast.makeText(v.getContext(), "position: "+position,Toast.LENGTH_LONG).show(); //밑에  홈버튼위에 포지션값이나온다

                    //아이템을 클릭하면 해당 세부 정보 액티비티로 넘겨주기
                    Intent intent = new Intent(v.getContext(),PhonebookDetail.class);//액티비티가아니라 App~~쓰지못하고 ,v.getConetxt를 사용했다 .
