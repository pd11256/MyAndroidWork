...
public class MySQLiteOpenHelper4 extends SQLiteOpenHelper {
    ...
    //데이터 베이스의 '버전' 이 바뀌었을떄 호출되는 콜백 메소드
    //'버전' 이 바뀌었을때 기존에 설치 운영되고 있는 데이터베이스 어떻게 변경할 것인지 작성
    // 각 버전의 변경 내용들을 버젼마다 작성해야 함.
    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        Log.d("myapp","SQLiteOpenHelper] onUpgrade() 호출: "+oldVersion+"->"+newVersion);
        //TODO 기존의 테이블 없애고 새로만든다.

        String sql ="DROP TABLE IF EXISTS student"; //기존 테이블 삭제,오라클에서는 못쓴다
        db.execSQL(sql);
